...
With[{problem=If[Head[oproblem]===Association, PiecewiseExpandAssociation[oproblem], oproblem]},
  Which[
   problem === $Failed,
   $Failed,
...
SimplifyPositivityOddStepOperator[variables_Association][problem_] :=
  ...
  (* high variables *)
     ...
     candidatevariableshigh = If[oddcandidateshigh=!={}, DeleteDuplicates[oddcandidateshigh[[All, 2]]],{}];
  ...
  (* low variables *)
     ...
     candidatevariableslow = If[oddcandidateslow=!={},DeleteDuplicates[oddcandidateslow[[All, 2]]],{}];
...
(* single variables *)
     ...
     candidatevariablessingle =  If[oddcandidatessingle=!={},DeleteDuplicates[oddcandidatessingle[[All, 2]]],{}];
...
evencandidatevariableshigh = If[evencandidateshigh==={},{},DeleteDuplicates[evencandidateshigh[[All, 2]]]];
...
evencandidatevariableslow = If[evencandidateslow==={},{},DeleteDuplicates[evencandidateslow[[All, 2]]]];
...
evencandidatevariablessingle = If[evencandidatessingle==={},{},DeleteDuplicates[evencandidatessingle[[All, 2]]]];
...
oddcandidatevariableshigh = If[oddcandidateshigh==={},{},DeleteDuplicates[oddcandidateshigh[[All, 2]]]];
...
oddcandidatevariableslow = If[oddcandidateslow==={},{},DeleteDuplicates[oddcandidateslow[[All, 2]]]];
...
oddcandidatevariablessingle = If[oddcandidatessingle==={},{},DeleteDuplicates[oddcandidatessingle[[All, 2]]]];
...
findExtrema[function_, vars_List, pars_List: {}, OptionsPattern[]] := 
 Module[{randomSampleSize, type, facts, usedVars, usedPars, 
  ...
  {parametricFacts, nonparametricFacts} = 
   GroupBy[If[FreeQQ[facts, And], List@facts, List @@ facts], 
  ...
  nonparametricFacts}];*)
  typeValue = If[type === Min, MinValue, MaxValue];
  directExtrema = 
   typeValue[{function, And @@ nonparametricFacts}, usedVars];
  If[Head@directExtrema =!= typeValue, Return@directExtrema];
  randomParValues = 
   Through[Unevaluated@
...
AssociatedPolynomial[exp_List, depVars_List, indVars_List, 
 ...
 Module[{print, facts, expr, vars, functions, extraFunctions, 
   extraFacts}, print = DebugPrint[False, "[assoc-pol]"];
  facts = OptionValue[Facts];
  print["facts = ", facts];
  expr = Together /@ exp;
  ...
  If[functions == {}, Return[{expr, {}, facts}]];
...
\[GreaterEqual]0*)
   ...
   If[vars === {}, Return[cleanup[And @@ (# >= 0 & /@ expr)]]];
   print["vars = ", vars];
      ...
      FullSimplify@
       Reduce[Exists[formalCoeffs, 
         ForAll[Evaluate@Union[allVars, extraFunctions], 
          And @@ nonparametricFacts && extraFacts, 
          And @@ parametricFacts && expr >= 0]], dom], 
      timeConstraint, $Failed]],(*If type===ForAll*)
    If[And @@ (PolynomialQ[#, vars] & /@ expr), 
     print["Given expression seems to be a polynomial."];
