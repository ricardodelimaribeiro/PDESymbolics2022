...
PiecewiseOperatorMap[Operator_, variables_, XPO_] := 
 With[{XP = PiecewiseExpand[XPO]},
  Which[
   ...
   $Failed,
   Head[XP] === Piecewise,
   With[{G = PiecewiseLastCaseClean[XP],domain = Lookup[variables,"domain",Reals]},
    With[{qq = Reduce[#,domain]& /@ MapThread[Simplify[! #1 && #2] &,
      ...
      Module[{gg},
       ...
       Piecewise[{If[#[[1]] =!= $Failed, 
            Operator[
              Append[variables, 
               "facts" -> Reduce[Lookup[variables, "facts", 
                  True] && #[[2]],domain]
...
Options[ConditionalPiecewiseMap]={domain->Reals};

ConditionalPiecewiseMap[F_,XP_,OptionsPattern[]] :=
Which[
	XP === $Failed, 
		$Failed,		
	Head[XP]=!=Piecewise,
...
PiecewiseBeautify[P_(*,OptionsPattern[]*)] :=
    Which[
        ...
        With[ {pp = PiecewiseLastCaseClean[P]},
            Module[ {qq(*,dom=OptionValue[domain]*)},
            (*Print["PiecewiseBeautify: ",dom];*)
                qq = Reduce[#,Reals]&/@MapThread[Simplify[!#1&&#2]&, {FoldList[Or,False, pp[[All,2]]][[;;-2]],pp[[All,2]]}];
...
Clear[PiecewiseEliminateEqualitiesOperator];
