...
label = "list with 2"
options = <|"pars"->{},"result"->{Subscript[\[FormalA], 1] u[x] + 
...
options = <|"pars"->{},"coeff" -> \[FormalC], "result"->{Subscript[\[FormalC], 1] u[x] + 
...
options = <|"pars"->{},"unique"->True, "result"->{Subscript[\[FormalA], 1] u[x] + 
  Subscript[\[FormalA], 2] Derivative[1][u][x], {Subscript[\[FormalA],
   1], Subscript[\[FormalA], 2]}}|>;
list = {u[x], u'[x]};
Get[test]
